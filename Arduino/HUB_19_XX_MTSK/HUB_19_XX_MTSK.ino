  ...
  if (node != 99999) //if the node number has been found
  {
    ...
    switch (RxData[0]) {
      ...
      case SMCMD: //smartlight command
        ...
        aSMCommand[1] = RxData[1]; //pin
        aSMCommand[2] = RxData[2]; //status
        aSMCommand[3] = RxData[3]; //value
        aSMCommand[4] = RxData[4]; //color/mood
        sendCommandOnSerial();
        break;
    }
...
int sendRemoteCommand(int node) // n=node
{
  ...
  */
  ...
  //if (xbee.readPacket(receiveResponseTimeout)) {
  //    break;
  //}
  // }
  ...
  for (int c = 0; c < 3; c++) { //try 3 times to receive the right response
    ...
    else if (x == RX_RESPONSE) //if it is a RX packet
    {
      processRx(x);
    }
