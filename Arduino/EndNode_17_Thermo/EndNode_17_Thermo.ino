...
SoftwareSerial nextion(10, 11);// Nextion TX to pin 11 and RX to pin 10 of Arduino
Nextion HMISerial(nextion, 57600); //create a Nextion object named myNextion using the nextion serial port @ 9600bps

unsigned int timeline[145];
unsigned int range[5][2];
unsigned int weekdays[8][3];
...
#define TIMEt0 2000 //update sensor data
#define TIMEt1 3000 //send data to coordinator
#define TIMEt2 30000 //read and update DHT sensor data
...
//leggo le temperatura interna
     ...
     Tmedia =  round((Ttotal/10.0)+OFFSET);
     Serial.println(seq);
     arr[seq] = Tmedia;  
     ...
     Tmedia =  round(ttotM/maxseq); 
     TempKelvinInt = float(Tmedia) * 480 / 1023;//480 sono i mV effettivi in ingresso al LM335
     TempCelsiusInt = round((TempKelvinInt-273.15)*10);
...
/*
  ...
  R = 100000.0 * R;
  t_intTemp = (1.0 / (log(R / 100000.0) / B + 1 / 298.15) - 273.15); //convert to temperature via datasheet ;
  t_intTemp = round(t_intTemp * 10) / 10;
  /*
*/
