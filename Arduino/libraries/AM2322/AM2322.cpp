/*************************************************** 
  This is a library for the AM2322 Humidity & Temp Sensor
...
boolean AM2322::readData(void) {
  uint8_t reply[10];
  
  // Wake up the sensor
  Wire.beginTransmission(AM2322_I2CADDR);
  delay(2);
  Wire.endTransmission();

  // OK lets ready!
  Wire.beginTransmission(AM2322_I2CADDR);
  Wire.write(AM2322_READREG);
  ...
  Wire.requestFrom(AM2322_I2CADDR, 8);
  for (uint8_t i=0; i<8; i++) {
    reply[i] = Wire.read();
    //Serial.println(reply[i], HEX);
  }
  
  if (reply[0] != AM2322_READREG) return false;
...
float AM2322::readTemperature(void) {
  if (! readData()) return NAN;
  return temp;
}

float AM2322::readHumidity(void) {
  if (! readData()) return NAN;
  return humidity;
}
 ...
 */
bool AM2322::readTemperatureAndHumidity(float &t, float &h) {
